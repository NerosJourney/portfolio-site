...
const LeftTimelineExperience = ({ role, date, company, desc, logo }) => {
  return (
    <div className="my-10 py-10 relative bg-slate-700">
      ...
      <p className="absolute top-[40%] text-cyan-100 font-mono left-[60vw] text-5xl">
        {company}
      </p>
    </div>
  );
...
const LeftExperiencePopout = ({ role, date, desc }) => {
  return (
    <div className="peer-hover:visible hover:visible invisible text-left absolute top-0 bottom-0 left-12 right-[60vw] m-2 p-2 bg-slate-800 border-rose-500 border-2 rounded-lg">
  ...
  return (
    <div className="my-10 py-10 relative ">
      <img
        ...
        alt={company}
      />
      <RightExperiencePopout
        role={role}
        date={date}
        desc={desc}
        company={company}
      />
      <p className="absolute top-[40%] text-cyan-100 font-mono right-[60vw] text-5xl">
        {company}
      </p>
    </div>
  );
...
const RightExperiencePopout = ({ role, date, desc }) => {
  return (
    <div className="peer-hover:visible hover:visible invisible text-left absolute top-0 bottom-0 right-12 left-[60vw] m-2 p-2 bg-slate-800 border-rose-500 border-2 rounded-lg">
      <p className="text-2xl md:text-5xl text-cyan-500 rounded-r-lg rounded-b-none border-b-rose-500 border-b-2 font-mono">
        {role}
      </p>
      <p className="text-lg md:text-3xl text-cyan-500 font-mono">{date}</p>
